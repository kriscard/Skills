> **Read this when:** writing assertions, using matchers, asymmetric matchers, soft assertions, poll assertions, spy/mock assertions, or extending with custom matchers.
...
expect({ value: 1 }).toEqual({ value: expect.anything() }) // anything except null/undefined
...
expect.soft(2).toBe(3) // also runs; all failures reported at end
```
