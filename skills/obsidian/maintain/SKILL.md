...
name: maintain
description: >-
  Runs a read-only health check on the Obsidian vault — finds broken links,
  orphaned notes, tag inconsistencies, and wiki issues, then reports
  prioritized fixes. Make sure to use this skill whenever the user says "vault
  health check", "fix broken links", "clean up my vault", "vault maintenance",
  or runs /maintain. Never modifies anything without explicit request — this
  skill diagnoses, it doesn't operate.
user-invocable: true
---

# Vault Maintenance

Read-only health audit. Diagnoses issues across four dimensions: wiki quality,
link integrity, tag consistency, and overall stats. Reports findings with
prioritized fixes — then waits for the user to decide what to act on.
...
After wiki lint, append findings summary to `log.md`:
```bash
obsidian patch path="3 - Resources/log.md" section="## $(date +%Y-%m-%d)" \
  content="lint | <summary of findings>"
```
...
Scan for:
- **Redundant tags**: tags that duplicate folder location (e.g., `#project`
  on a note already in `1 - Projects/`)
- **Notes exceeding 4-tag limit**: more than 4 tags is a signal of unclear
  categorization
- **TIL notes missing `til/` prefix**: any tags in `3 - Resources/TIL/`
  notes that don't follow the `til/<tech>` convention
